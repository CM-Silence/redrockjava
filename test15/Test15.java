...
public class Test15 {
    ...
    public static String judge(){
        do {
            ...
            formula = formula.replaceAll("【","[");
            formula = formula.replaceAll("】","]");
            formula = formula.replaceAll("pi",Double.toString(2 * Math.asin(1))); //将圆周率换成数字
    ...
    //一个中括号或一个小括号或一个数字(旧版):(((-)?\[(((-)?(\d)+(\.(\d)+)?)|((-)?\((-)?(\d)+(\.(\d)+)?([+\-*/^](-)?(\d)+(\.(\d)+)?)+\)))
    // ([+\-*/^](((-)?(\d)+(\.(\d)+)?)|((-)?\((-)?(\d)+(\.(\d)+)?([+\-*/^](-)?(\d)+(\.(\d)+)?)+\))))+])|
    // (((-)?(\d)+(\.(\d)+)?)|((-)?\((-)?(\d)+(\.(\d)+)?([+\-*/^](-)?(\d)+(\.(\d)+)?)+\)))|((-)?(\d)+(\.(\d)+)?))

    //运算符跟中括号或小括号或数字组合多次(旧版):([+\-*/^](((-)?\[(((-)?(\d)+(\.(\d)+)?)|((-)?\((-)?(\d)+(\.(\d)+)?([+\-*/^](-)?(\d)+(\.(\d)+)?)+\)))
    //([+\-*/^](((-)?(\d)+(\.(\d)+)?)|((-)?\((-)?(\d)+(\.(\d)+)?([+\-*/^](-)?(\d)+(\.(\d)+)?)+\))))+])|
    //(((-)?(\d)+(\.(\d)+)?)|((-)?\((-)?(\d)+(\.(\d)+)?([+\-*/^](-)?(\d)+(\.(\d)+)?)+\)))|((-)?(\d)+(\.(\d)+)?)))+

    //带有中括号、小括号和数字的算式(这就是上面那条正则表达式的内容啦)(将一部分+改成了*,现在支持表达式或括号里只有一个数字了!):
    //(((-)?\[(((-)?(\d)+(\.(\d)+)?)|((-)?\((-)?(\d)+(\.(\d)+)?([+\-*/^](-)?(\d)+(\.(\d)+)?)*\)))
    //([+\-*/^](((-)?(\d)+(\.(\d)+)?)|((-)?\((-)?(\d)+(\.(\d)+)?([+\-*/^](-)?(\d)+(\.(\d)+)?)*\))))*])|
    //(((-)?(\d)+(\.(\d)+)?)|((-)?\((-)?(\d)+(\.(\d)+)?([+\-*/^](-)?(\d)+(\.(\d)+)?)*\)))|((-)?(\d)+(\.(\d)+)?))
    //([+\-*/^](((-)?\[(((-)?(\d)+(\.(\d)+)?)|((-)?\((-)?(\d)+(\.(\d)+)?([+\-*/^](-)?(\d)+(\.(\d)+)?)*\)))
